...
const checkValue = (value, placeholder = "Not available") => {
...
const loadCategoryButtons = (categories) => {
  ...
  categories.forEach((item) => {
    ...
    categoryContainer.innerHTML = `
            <a id="category-${categoryInLC}" onclick="loadActiveBtn('${categoryInLC}')" class="flex items-center justify-center gap-1 md:gap-3 px-3 py-2 md:px-4 md:py-3 md:w-[168px] lg:w-[280px] rounded-lg border-2 border-gray-200 hover:bg-gray-100 transition duration-200 ease-in-out">
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (!data.data || data.data.length === 0) {
        ...
        // Clear allPets to indicate empty category
        allPets = [];
      ...
      // When pets are found
      allPets = [...data.data]; // Update allPets when data exists
...
sortBtn.addEventListener("change", function () {
  ...
  // Prevent sorting if the category is empty
    ...
    petCard.innerHTML = `
      <div class="card bg-base-100 shadow-sm">
        <figure class="overflow-hidden h-48 shadow-lg">
          <img src="${checkValue(pet.image, 'images/default.jpg')}" alt="${checkValue(pet.category)}" class="object-cover w-full h-full" />
        </figure>
        <div class="p-6 md:p-4">
          <h2 class="card-title mb-2">${checkValue(pet.pet_name)}</h2>
          <p class="text-gray-500 text-base">Breed: ${checkValue(pet.breed)}</p>
          <p class="text-gray-500 text-base">Birth: ${checkValue(pet.date_of_birth)}</p>
          <p class="text-gray-500 text-base">Gender: ${checkValue(pet.gender)}</p>
          <p class="text-gray-500 text-base">Price: ${checkValue(pet.price)}</p>
          <div class="card-actions justify-between">
            <button id="likeCard${pet.petId}" class="likeBtn btn bg-white mt-3 px-4 py-2 rounded-lg">
              <svg id="likeIcon${pet.petId}" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="none">
                ...
                stroke="${isLiked ? "#FF0000" : "#131313"}" class="${isLiked ? "primary-stroke" : ""}" />
              </svg>
            </button>
            <button onclick="adoptPet('${checkValue(pet.pet_name)}', this)" class="btn bg-white primary-color mt-3 px-5 py-2 rounded-lg">Adopt</button>
...
const displayPetDetails = (pet) => {
  ...
  modalCard.innerHTML = `
    <div class="rounded-xl bg-base-100">
      ...
      <div class="mt-5">
        <h2 class="card-title">${checkValue(pet.pet_name)}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-1 pt-4 pb-4 border-b border-gray-200">
          <p class="flex items-center gap-2 text-gray-500 text-base">
            <img src="images/icons/Frame.svg" alt="icon" class="w-5 h-5"> Breed: ${checkValue(pet.breed)}
          </p>
          <p class="flex items-center gap-2 text-gray-500 text-base">
            <img src="images/icons/Frame (1).svg" alt="icon" class="w-5 h-5"> Birth: ${checkValue(pet.date_of_birth)}
          </p>
          <p class="flex items-center gap-2 text-gray-500 text-base">
            <img src="images/icons/Frame (2).svg" alt="icon" class="w-5 h-5"> Gender: ${checkValue(pet.gender)}
          </p>
          <p class="flex items-center gap-2 text-gray-500 text-base">
            <img src="images/icons/Frame (3).svg" alt="icon" class="w-5 h-5"> Price: ${checkValue(pet.price)}
          </p>
          <p class="flex items-center gap-2 text-gray-500 text-base">
            <img src="images/icons/Frame (2).svg" alt="icon" class="w-5 h-5"> Vaccinated status: ${checkValue(pet.vaccinated_status)}
          </p>
        </div>
        <div class="pt-4">
          <h5 class="font-semibold">Details information: </h5>
          <p class="pt-3">${checkValue(pet.pet_details)}</p>
        </div>
      </div>
    </div>
  `;
